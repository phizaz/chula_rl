...
class SubprocVecEnv(BaseVecEnv):
    ...
    def __init__(self, env_fns, spaces=None):
        ...
        # obs types
        keys, shapes, dtypes = obs_space_info(observation_space)
        self.dtypes = dtypes

        self.viewer = None
        self.specs = [f().spec for f in env_fns]
        BaseVecEnv.__init__(self, len(env_fns), observation_space, action_space)
    ...
    def step_wait(self):
        ...
        return (
            _flatten_obs(obs, self.dtypes),
            np.stack(rews).astype(np.float32),
            np.stack(dones).astype(np.bool),
            infos,
        )
    ...
    def reset(self):
        ...
        return _flatten_obs([remote.recv() for remote in self.remotes], self.dtypes)
...
def _flatten_obs(obs, dtypes):
    ...
    if isinstance(obs[0], dict):
        ...
        return {k: np.stack([o[k] for o in obs]).astype(dtypes[k]) for k in keys} 
    else:
        return np.stack(obs).astype(dtypes[None])
